...
struct TRSAKeys {
    std::pair<uint1024, uint1024> PublicKey; // (e, n)
    std::pair<uint1024, uint1024> PrivateKey; //  (d, n)
...
namespace {
	...
	Тогда для любого a из Z_n выполняется хотя бы одно из условий:
	1. a^d equiv 1 (mod n)
	2. Существует целое число r<s такое что  a^(d * 2^r) equiv -1 (mod n)
	*/
    ...
    bool IsPrime(uint1024 value) {
        ...
        ++value;
        uint1024 q{value / (temp << power)}; // temp * 2^power
...
TRSAKeys GenerateRSAKeys(const uint1024& p, const uint1024& q, int bitsCount) {
    uint1024 moduleValue = p * q; // n
    uint1024 phi = (p - 1) * (q - 1); // Euler function

    uint1024 openExp; // 1 < e < phi, взаимно простое с phi
    uint1024 privateExp = -1; // d * e mod phi = 1
...
int main() {
	constexpr int bitsCount = 1024; // random number size

    auto p = GeneratePrime(bitsCount); // prime number p
    auto q = GeneratePrime(bitsCount); // prime number q
    
    auto rsaKeys = GenerateRSAKeys(p, q, bitsCount); // private & public keys
    PrintKeys(rsaKeys);
